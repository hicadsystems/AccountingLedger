        ...
        public async Task<UploadResponse> processUploadInThread()
        {
            ...
            try
            {
                ...
                foreach (var s in loanCaptures)
                {
                    ...
                    if(getPersonId == null)
                    {
                        response.Errors.Add($"Service Number {s.SVC_NO} not found");
                        continue;
                    }
                    ...
                    if (getLoanType == null)
                    {
                        response.Errors.Add($"Loan type {s.LOAN_TYPE} not found");
                        continue;
                    }
                    ...
                    var getloanregister = unitOfWork.loanRegisterRepository
                        .GetloanregisterByCode(x => x.PersonID == getPersonId.PersonID && x.batchNo == s.BATCH_NO + '_' + getLoanType.Description);
                    if (getloanregister != null)
                    {
                        response.Errors.Add($"SERVICE NO {s.SVC_NO} with Batch No {s.BATCH_NO + '_' + getLoanType.Description} Already Exist");
                        continue;
                    }
